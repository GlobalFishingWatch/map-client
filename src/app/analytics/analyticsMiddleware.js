...
const GA_ACTION_WHITELIST = [
  {
    ...
    category: 'Layer',
    action: (action, state) => {
      const layerIndex = state.layers.workspaceLayers.findIndex(
        (l) => l.id === action.payload.layerId
      )
      const changedLayer = state.layers.workspaceLayers[layerIndex]
      const isVisible =
        action.payload.forceStatus !== null ? action.payload.forceStatus : !changedLayer.visible
  ...
  },
  {
    ...
    getPayload: ({ payload }) => {
      const filters = payload.flagFilters.filter(
        (flagFilter) => typeof flagFilter.flag !== 'undefined'
      )
      if (filters.length) {
  ...
  },
  {
    ...
    getPayload: (action, state) =>
      `${state.report.layerId}:${state.report.currentSelectedPolygon.reporting_name}`,
  },
  {
    ...
    action: 'Add polygon to report',
    getPayload: (action, state) =>
      `${state.report.layerId}:${state.report.polygons[action.payload.polygonIndex].name}`,
  },
  {
    ...
    getPayload: (action, state) =>
      `${state.report.layerId}:${state.report.polygons.map((elem) => elem.name).join(':')}`,
  },
  {
