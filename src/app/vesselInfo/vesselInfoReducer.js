...
import { PALETTE_COLORS, DEFAULT_TRACK_PALETTE_INDEX } from 'app/config'
...
export default function(state = initialState, action) {
  switch (action.type) {
    ...
    case SET_VESSEL_DETAILS: {
      ...
      return Object.assign({}, state, {
        vessels: [
          ...state.vessels.slice(0, vesselIndex),
          newVessel,
          ...state.vessels.slice(vesselIndex + 1),
        ],
    ...
    case TOGGLE_VESSEL_PIN: {
      ...
      if (action.payload.pinned === true) {
        currentPaletteIndex =
          state.currentPaletteIndex === PALETTE_COLORS.length - 1
            ? 0
            : state.currentPaletteIndex + 1
      }
      ...
      return Object.assign({}, state, {
        vessels: newVessels,
        pinnedVesselEditMode:
          state.pinnedVesselEditMode && newVessels.filter((e) => e.pinned === true).length > 0,
        currentlyShownVessel,
        currentPaletteIndex,
      })
    }
    ...
    case TOGGLE_PINNED_VESSEL_EDIT_MODE: {
      const newState = Object.assign({}, state, {
        pinnedVesselEditMode:
          action.payload.forceMode === null
            ? !state.pinnedVesselEditMode
            : action.payload.forceMode,
      })
