...
import BasemapStyles from 'styles/rightControlPanel/basemap-panel.module.scss'
...
class BasemapPanel extends Component {
  render() {
    ...
    basemaps.forEach((basemap) => {
      const urlThumbnail = `${process.env.PUBLIC_URL}/basemaps/${basemap.id}.png`
      const basemapButton = (
        <li
          className={classnames(
            ListItemStyles.listItem,
            ListItemStyles._noMobileRightPadding,
            ListItemStyles.halfRow,
    ...
    return (
      <div className={BasemapStyles.basemapsPanel}>
        <ul className={ListItemStyles.list}>{basemapButtons}</ul>
        <ul className={BasemapStyles.options}>{basemapOptionsButtons}</ul>
      </div>
    )
  }
}
