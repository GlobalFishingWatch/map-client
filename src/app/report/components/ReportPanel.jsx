...
import { REPORT_STATUS } from 'app/constants'
import iconStyles from 'styles/icons.module.scss'
import ReportPanelStyles from 'styles/components/map/report-panel.module.scss'

import { ReactComponent as RemovePolygonIcon } from 'assets/icons/remove.svg'
...
class ReportPanel extends Component {
  ...
  render() {
    ...
    const panelClass =
      this.state.expanded && window.innerWidth >= 1024
    ...
    const containerClass = this.state.expanded
      ? classnames(ReportPanelStyles.container, ReportPanelStyles._expanded)
      : ReportPanelStyles.container

    const toggleClass = this.state.expanded
      ? ReportPanelStyles.toggle
      : classnames(ReportPanelStyles.toggle, ReportPanelStyles._expanded)

    let content

    if (this.props.status === REPORT_STATUS.sent || this.props.status === REPORT_STATUS.error) {
      content = (
        <li className={ReportPanelStyles.polygonItem}>
      ...
      this.props.polygons.map((polygon, index) =>
        content.push(
          <li className={ReportPanelStyles.polygonItem} key={polygon.reportingId}>
      ...
      content = (
        <li className={ReportPanelStyles.polygonItem}>
          <span className={ReportPanelStyles.polygonName}>
            No regions added yet.
            <br /> Select regions on the map.
          </span>
        </li>
      )
    }
    ...
    if (this.props.status === REPORT_STATUS.idle || this.props.status === REPORT_STATUS.error) {
      buttons = (
        <div className={ReportPanelStyles.reportOptions}>
          <button
          ...
          <button className={ReportPanelStyles.reportButton} onClick={this.props.onDiscardReport}>
            discard
          </button>
        </div>
      )
      ...
      buttons = (
        <div className={ReportPanelStyles.reportOptions}>
          <button
            className={classnames(ReportPanelStyles.reportButton, ReportPanelStyles._wide)}
            onClick={this.props.onReportClose}
          >
            close
          </button>
        </div>
    ...
    return (
      <div className={panelClass}>
        <div className={ReportPanelStyles.menu} onClick={() => this.onTogglePanel()}>
          <span className={ReportPanelStyles.reportTotal}>
            {this.props.layerTitle}: {this.props.polygons.length} added
          </span>
          <span className={toggleClass} />
        </div>
        <div className={containerClass}>
          <div className={ReportPanelStyles.content}>
            <ul className={ReportPanelStyles.polygonList}>{content}</ul>
            {this.props.reportableInfo.hasNonReportableLayers && (
              <div className={ReportPanelStyles.warning}>
                ...
                {this.props.reportWarning.replace(
                  '$REPORTABLE_LAYERS',
                  this.props.reportableInfo.reportableLayersNames
                )}
              </div>
            )}
          </div>
