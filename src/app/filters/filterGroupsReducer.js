...
import { PALETTE_COLORS_LAYERS } from 'app/config'
  ...
  switch (action.type) {
    case CREATE_NEW_FILTER_GROUP: {
      return Object.assign({}, state, {
        ...
        editFilterGroupIndex: null,
        defaultColorIndex:
          state.defaultColorIndex === PALETTE_COLORS_LAYERS.length - 1
            ? 0
            : state.defaultColorIndex + 1,
      })
    }
    case SET_EDIT_FILTER_GROUP_INDEX: {
      ...
      const currentlyEditedFilterGroup =
        editFilterGroupIndex === null
          ? null
          : Object.assign({}, state.filterGroups[editFilterGroupIndex])
      return Object.assign({}, state, {
    ...
    case SET_FILTER_GROUP_MODAL_VISIBILITY: {
      return Object.assign({}, state, {
        isFilterGroupModalOpen: action.payload,
      })
    }
    case SAVE_FILTER_GROUP: {
      ...
      if (index !== null) {
        newFilterGroups = [
          ...state.filterGroups.slice(0, index),
          action.payload.filterGroup,
          ...state.filterGroups.slice(index + 1),
        ]
      } else {
    ...
    case SET_FILTER_GROUP_VISIBILITY: {
      ...
      const newFilterGroup = Object.assign({}, state.filterGroups[index], {
        visible,
      })
    ...
    case DELETE_FILTER_GROUP: {
      ...
      return Object.assign({}, state, {
        filterGroups: [
          ...state.filterGroups.slice(0, action.payload),
          ...state.filterGroups.slice(action.payload + 1),
        ],
      })
    }
    ...
    case SET_CURRENT_FILTER_GROUP_ACTIVE_LAYER: {
      ...
      currentlyEditedFilterGroup.checkedLayers[action.payload.layerId] = !currentlyEditedFilterGroup
        .checkedLayers[action.payload.layerId]
      return Object.assign({}, state, {
        currentlyEditedFilterGroup,
      })
    }
    ...
    case SET_CURRENT_FILTER_VALUE: {
      ...
      if (!action.payload.values.length || action.payload.values[0].id === '') {
        ...
        currentlyEditedFilterGroup.filterValues[action.payload.id] = action.payload.values.map(
          (v) => parseInt(v.id, 10)
        )
      }
      return Object.assign({}, state, {
