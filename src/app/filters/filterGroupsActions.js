...
import { trackCreateFilterGroups } from 'app/analytics/analyticsActions'
...
export function createNewFilterGroup() {
  return (dispatch, getState) => {
    const checkedLayers = {}
    getState()
      .layers.workspaceLayers.filter((elem) => elem.type === LAYER_TYPES.Heatmap)
      .map((l) => l.id)
      .forEach((lid) => {
  ...
  filters
    ...
    .forEach((filterGroup) => {
      ...
      Object.keys(filterGroup.filterValues).forEach((filterValueKey) => {
        ...
        const originalLayerHeaderFilter = heatmapLayer.header.filters.find(
          (layerHeaderFilter) => layerHeaderFilter.id === filterValueKey
        )
...
export function setLayerFilters() {
  return (dispatch, getState) => {
    ...
    const heatmapLayers = getState().layers.workspaceLayers.filter(
      (layer) => layer.type === LAYER_TYPES.Heatmap && layer.added === true
    )
...
export function saveFilterGroup(filterGroup = null) {
  return (dispatch, getState) => {
    ...
    const newFilterGroup =
      filterGroup || Object.assign({}, getState().filterGroups.currentlyEditedFilterGroup)
