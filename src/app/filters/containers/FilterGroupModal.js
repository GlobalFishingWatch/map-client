...
import FilterGroupModal from 'app/filters/components/FilterGroupModal'
import { LAYER_TYPES, FLAG_FILTER_GROUP_VALUES } from 'app/constants'
import { saveFilterGroup, setFilterGroupModalVisibility } from 'app/filters/filterGroupsActions'

const mapStateToProps = (state) => {
  const currentlyEditedFilterGroup = state.filterGroups.currentlyEditedFilterGroup
  const filterValuesKeys =
    currentlyEditedFilterGroup && Object.keys(currentlyEditedFilterGroup.filterValues)
  ...
  const layers = state.layers.workspaceLayers
    ...
    .map((l) => {
      l.filterActivated =
        currentlyEditedFilterGroup && currentlyEditedFilterGroup.checkedLayers[l.id] === true
      return l
    })

  // prepare filters: dedupe filters that have the same id, set default values
  const filteredLayers = layers.filter(
    (l) => l.filterActivated === true && l.header.filters !== undefined
  )
  ...
  const anyLayerChecked =
    currentlyEditedFilterGroup &&
    Object.keys(currentlyEditedFilterGroup.checkedLayers).some(
      (key) => currentlyEditedFilterGroup.checkedLayers[key] === true
    )
  const anyFilterSelected =
    currentlyEditedFilterGroup && Object.keys(currentlyEditedFilterGroup.filterValues).length > 0
  ...
  const defaultLabel = !filters.length
    ? ''
    : filterValuesKeys
        .map((filterId) => {
