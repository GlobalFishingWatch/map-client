...
export const updateMapStyle = () => (dispatch, getState) => {
  const state = getState();
  const staticAndCustomLayers = state.layers.workspaceLayers.filter(layer => LAYER_TYPES_MAPBOX_GL.indexOf(layer.type) > -1);  // OUTSIDE

  // read basemap info from workspace
  const basemapLayers = state.basemap.basemapLayers;  // OUTSIDE
  ...
  for (let i = 0; i < glLayers.length; i++) {
    ...
    const layerIsInReport = refLayer.id === state.report.layerId; // OUTSIDE
